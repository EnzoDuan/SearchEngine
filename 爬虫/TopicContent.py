...
#        time.sleep(0.1)
        ...
        for content,authors_all,authors_href,authors_intro, href_t, title in \
            zip(contents, authors_alls,authors_hrefs,authors_intros, hrefs, titles):
...
# End one page
        del hrefs, contents, authors_alls,authors_hrefs,authors_intros, titles,html2, soup1
        ...
        if(len(pages) == 0):
            ...
            file.close()
            return 
        if(((count == 2) & (pages[0].text != "上一页")) | (count == 0) | (pages[0].text == "16")):
            print('end ' + filename)
            del pages, count
            file.close()
            return
